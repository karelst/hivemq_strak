...
global client
#global connection_ok  #connection_ok
connection_ok = False

# Define the MQTT callback functions
def on_connect( clie, userdata, flags, rc):
    global client
    client = clie
...
def on_message(receivng_client, userdata, msg):
    #print (receiving_client)
    if msg.topic != 'isalive' and msg.topic != 'WillTopic':
...
def connect_broker():
    ...
    client.loop_start()
    time.sleep(1)
    connection_ok = client.is_connected()

        
def check_connection():
    global client, connection_ok
    connection_ok = client.is_connected()
